...
echo -e "command: \033[0;31mparted /dev/sda mklabel gpt\033[0;0m"
parted /dev/sda mklabel gpt && echo done
echo
...
parted /dev/sda mkpart ESP fat32 1MiB 513MiB && parted /dev/sda set 1 boot on && parted /dev/sda mkpart primary ext4 513MiB 20.5GiB && parted /dev/sda mkpart primary linux-swap 20.5GiB 36.5GiB && parted /dev/sda mkpart primary ext4 36.5GiB 100%
...
echo "Do we have the right mirrors in place?"
echo "Make the right changes to /etc/pacman.d/mirrorlist"
read check

# do this instead, using the cache server makes things SO FAST
#cp /root/arch/fastList /etc/pacman.d/mirrorlist

# 5. install the base system
echo -e "\033[0;34m5. install the base system\033[0;0m"
echo -e "command: \033[0;31mpacstrap -i /mnt base base-devel\033[0;0m"
